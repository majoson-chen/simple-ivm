        ...
        if goods.flag:
            if goods.flag == 'add':
                ...
                try:
                    ...
                    # 无法删除, 可能是新建分类的时候添加了删除标记
                    pass
        else:
            # 修改
            inst: Goods = Goods.get_by_id(goods.key)
            inst.name = goods.name
            inst.quan = goods.quan
            inst.save()
            goods_ls.append(goods)
            
    return CategoryType(key = str(indb.key), name = indb.name, mark = indb.mark, unit = indb.unit, goods=goods_ls)
